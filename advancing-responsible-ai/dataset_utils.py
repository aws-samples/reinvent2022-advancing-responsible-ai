import os
from random import uniform, sample, choices, seed
import numpy as np
import pandas as pd
import torch
...
def create_mnist_tables():
    """
    Creates dataframe of the MNIST dataset in the format that they need to be in to start the workshop.
    In particular, images are thresholded, and each img corresponds to a single row with the following data:
        ['img', 'label', 'digit', 'parity', 'magnitude']
    where digit, parity, and magnitude are the groups we will assess bias on.
    """
    root = 'data'

    train_mnist = datasets.mnist.MNIST(root, train=True, download=True, transform=None, target_transform=None)
...
def create_colored_mnist_tables(p):
    """
    Creates dataframe of the MNIST dataset in the format that they need to be in to start the workshop,
    with specific color augmentations parameterized by input float (p). Colors are determined via the
    `color_data_stochastically_by_label` helper function which documents the specific methodology for coloring.
...
def _color_data_stochastically_by_label(df, img_col, label_col, p, random_seed=0):
    """
    Takes in a dataframe of images with labels 0-9 and for each image recolors it to have a black background
    with the digit d colored with the dth color with probability p and the d+1 (mod 10)th color with probability 1-p.
    """
    seed(random_seed)
    colored_df = df.copy()
    ...
    colored_df['color'] = colors  # Add the color column, since we went in order this should work fine
    return colored_df
...
def bicolor_data_stocastically_by_label(df, img_col, label_col, p, colorA='red', colorB='green', random_seed=0):
    ...
    Args: 
        ...
        p: float describing the proportion of images that will be colored red in the output DataFrame.
    Returns:
    ...
    for i, (img, digit) in enumerate(zip(df[img_col], df[label_col])):
        ...
        clr = colorA if (uniform(0, 1) > p) else colorB
...
def color_mnist_data_with_dict_per_digit(df, color_probs, random_seed=0):
    ...
    color_probs: dict[int, dict[string, float]]
        ...
        For instance, if we 3s to be 40% red and 60% green, then color_probs[3] should be `{'red': 0.4, 'green':0.6}`.
        The sum of values of each dictionary must be 1.0 or a ValueError will be raised.
        The valid colors are 'red', 'green', 'blue', 'yellow', 'cyan', 'magenta'
    """
    seed(random_seed)
    digit_dfs = []
    ...
    for d in range(10):
        ...
        try:
            ...
            raise ValueError(f'The sum of color_probs[d].values() must be 1.0 but for digit {d} it was '
                             f'{color_dist.values()}')

        # Copy the subset of the data corresponding to this digit
        digit_df = df[df['label'] == d].copy()
        # Determining the sampling list and weights for each instance (NOTE: this is random/noisy sampling)
        ...
        # for i, img in enumerate(digit_df['img']):
...
def resample_dataset(df, group_col, group_props, random_seed=0):
    ...
    For instance `subsample_dataframe_by_group(df, 'digit', [1., 1., 1., 1., 1., 1., 0.5, 1., 1., 1.])` would
    correspond to a dataset in which only half the original rows labeled 6 remain in the dataset.
    """
    # seed(random_seed)
    group_sizes = df[group_col].value_counts()
    group_weights = np.array(group_props) / max(group_props)  # N
    # Multiply the proportions (normalized so max is 1.0) by  min size to guarentee we have enough instances and
    # retain proportions of the data
...
def create_starting_data():
    ...
    and address rebalancing after the initial bias analysis.

    Both train_data and test_data have been augmented with parity, magnitude, curve, and color

    train_data:
    Distribution of identities is resampled to 0.3 3's and 0.3 8's. 
    This will affect accuracy on identity (3's 8's perform worse), on curve ('curvy' performs worse than 'linear')
    Color distribution will be .8 red / .2 green for all number identities, which may affect accuracy on color

    test_data:
    Distribution of identities ('label') is uniform
    color distribution is .5 red / .5 green for all number identities, 
    with the exception of 6 which has color distribution .2 red / .8 green, which may affect accuracy on 6's
...
def create_dataloader_from_df(df, img_col, label_col, dataset_name, data_shuffle_seed, train=True):
    ...
    data_loader = torch.utils.data.DataLoader(
        dataset,
        batch_size=64 if train else 1000,  # 64,
        shuffle=train  # only shuffle if training
    )
    return data_loader


def create_dataloader_from_df_with_groups(df, img_col, label_col, dataset_name, group_names, data_shuffle_seed,
                                          train=False):
    """
    Creates a pytorch dataloader from our dataframe.
    """
    dataset = make_dataset_from_df_with_groups(df, img_col, label_col, dataset_name, group_names)
    torch.manual_seed(data_shuffle_seed)
    data_loader = torch.utils.data.DataLoader(
        dataset,
        batch_size=64 if train else 1000,  # 64,
...
def make_dataset_from_df_with_groups(df, img_col, label_col, dataset_name, group_names, root='data'):
    """
    Makes an instance of the MNISTDataset class that also stores group membership values for each instance
    """
    root = os.path.join(root, dataset_name)
    data_tuples = list(zip(df[img_col], df[label_col], *(df[group] for group in group_names)))
...
class MNISTDataset(datasets.VisionDataset):
    """
    Dataset object that is spawned from our MNISTWrapper, we will create one of these for train and one for test.

    Attributes:
        data_tuples (list[Tuple[img, label, (group, parity, magnitude)]]): The raw data
    """
    def __init__(self, root, data_tuples, transform=transforms.ToTensor(), target_transform=None, group_names=()):
    ...
    def __getitem__(self, index):
        ...
        if len(self.group_names) > 0:
            # Note that we simply list the group values in indices 2,3,4,... so we need to use the * to pack into a
            # tuple
